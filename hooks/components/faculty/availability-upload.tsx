...
import { invalidateAllFacultyData } from "@/hooks/use-faculty";
  ...
  const uploadFile = async (confirm: boolean = false) => {
    ...
    try {
      ...
      if (data.requiresConfirmation) {
        ...
        // Invalidate all faculty caches so timetable/dashboard refresh
        await invalidateAllFacultyData();
            ...
            {result && !uploading && (
              <div className="space-y-3">
                ...
                {result.hasTimetableData && (
                  <div className="flex items-center gap-2 p-2 rounded-md bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
                    ...
                    <span className="text-xs font-medium text-blue-700 dark:text-blue-300">
                      Timetable entries detected ({result.timetableEntries} classes)
                    </span>
                  </div>
                )}
                ...
                {needsConfirm ? (
                  ...
                  <div className="flex flex-col items-center justify-center gap-2 p-3 rounded-lg bg-green-50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/20">
                    <div className="flex items-center gap-2 text-green-600 dark:text-green-400">
                      <Check className="h-5 w-5" />
                      <span className="text-sm font-medium">Upload Successful</span>
                    </div>
                    <p className="text-xs text-center text-muted-foreground">
                      {result.daysUpdated ?? 0} days updated
                      {result.timetableUpdated ? ` and ${result.timetableUpdated} timetable classes sync'd` : ""}
                    </p>
                  </div>
                ) : null}
              </div>
