...
export async function ensureFacultyExists(
  ...
  try {
    let faculty = await db.faculty.findUnique({
      where: { userId },
      include: { profile: true, preferredSubjects: true, department: true },
    });

    if (!faculty) {
      // Find or create a default department for auto-provisioned faculty
      ...
      if (!defaultDept) {
        defaultDept = await db.department.create({
          data: {
            name: "General",
            code: "GENERAL",
            description: "Default department for auto-provisioned faculty",
          },
        });
      }
