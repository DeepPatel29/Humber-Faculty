...
import { updateAvailabilitySchema, type UpdateAvailabilityInput } from "@/lib/validations/faculty";
import { DayOfWeek, PreferredSlot, type FacultyAvailability } from "@/lib/types/faculty";
...
interface AvailabilityFormProps {
	...
	onSave?: (data: UpdateAvailabilityInput & { eligibleCourseIds?: string[]; submitAsRequest?: boolean }) => Promise<void>;
	isSubmitting?: boolean;
}

export function AvailabilityForm({ availability, onSave, isSubmitting: externalSubmitting }: AvailabilityFormProps) {
	const [internalSubmitting, setInternalSubmitting] = useState(false);
	const isSubmitting = externalSubmitting ?? internalSubmitting;
	const [selectedCourses, setSelectedCourses] = useState<string[]>(availability?.eligibleCourseIds || []);
	const [hasChanges, setHasChanges] = useState(false);

	const availabilityDays = Array.isArray(availability?.days) ? availability.days : [];
	...
	const handleSubmit = async (data: UpdateAvailabilityInput, submitAsRequest: boolean) => {
		if (onSave) {
			await onSave({ ...data, eligibleCourseIds: selectedCourses, submitAsRequest });
		} else {
			...
			try {
				const res = await fetch("/api/faculty/availability", {
					...
					body: JSON.stringify({ ...data, eligibleCourseIds: selectedCourses, submitAsRequest }),
				});
				if (!res.ok) throw new Error("Failed to update availability");
				toast.success(submitAsRequest ? "Availability update request submitted for approval" : "Availability saved successfully");
				setHasChanges(false);
			} catch {
		...
		<form onSubmit={(e) => e.preventDefault()} className="space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Choose when you prefer to have classes scheduled</CardDescription>
				</CardHeader>
				<CardContent>
					<Controller
						name="preferredSlot"
						...
						render={({ field }) => (
							<RadioGroup
								value={field.value}
								onValueChange={(v) => { field.onChange(v); setHasChanges(true); }}
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Set specific hours if you have custom availability</CardDescription>
				</CardHeader>
				<CardContent>
					<FieldGroup>
						<div className="grid gap-4 sm:grid-cols-2">
							<Field>
								...
								<Input
									...
									onChange={(e) => { register("customStartTime").onChange(e); setHasChanges(true); }}
								/>
							...
							<Field>
								...
								<Input
									...
									onChange={(e) => { register("customEndTime").onChange(e); setHasChanges(true); }}
								/>
						...
						<div>
							<p className="mb-3 text-sm font-medium">Unavailable Period (e.g., lunch break)</p>
							<div className="grid gap-4 sm:grid-cols-2">
								<Field>
									...
									<Input
										...
										onChange={(e) => { register("unavailableStart").onChange(e); setHasChanges(true); }}
									/>
								...
								<Field>
									...
									<Input
										...
										onChange={(e) => { register("unavailableEnd").onChange(e); setHasChanges(true); }}
									/>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Select courses you are qualified and available to teach. This information will be used by schedulers.
					</CardDescription>
				</CardHeader>
				<CardContent>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Any special scheduling requirements or preferences</CardDescription>
				</CardHeader>
				<CardContent>
					<Textarea
						{...register("notes")}
						onChange={(e) => { register("notes").onChange(e); setHasChanges(true); }}
